...
func newProWatcher(cfg *config.Config, clip *clipv1.Server, controlled *bridge.ControlledSet, bridgeIP string, skipTLS bool, log *slog.Logger) *proWatcher {
	w := &proWatcher{
		cfg:        cfg,
		clip:       clip,
	...
	w.applyProvider = func(p *config.BridgePro) {
		clip.SetLightProvider(newProvider(bridgepro.New(p), controlled, log))
	}
	return w
}
